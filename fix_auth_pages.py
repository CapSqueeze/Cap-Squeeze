...
# The previous script mistakenly copied newsletter.html which had FAQ content inside it or didn't replace correctly because the markers were missing.
# Let's fix login.html and signup.html by reading from shipping.html which we know has the correct markers and layout, and replacing the content.

# 1. Read base template (we know shipping.html has the correct layout)
with open('shipping.html', 'r', encoding='utf-8') as f:
    template = f.read()
...
login_content = auth_css + '''
  <!-- PAGE HERO (Hidden for Auth pages, we just use the container) -->
...
signup_content = auth_css + '''
  <!-- PAGE HERO (Hidden for Auth pages) -->
  <section style="min-height: 80vh; display: flex; align-items: center; justify-content: center; background: var(--bg-surface);">
    <div class="auth-container">
...
def write_auth_page(filename, title, desc, html_content):
    content = template
    content = content.replace('<title>Shipping Info — Cap Squeeze</title>', f'<title>{title}</title>')
    content = content.replace('<meta name="description" content="Shipping processing times, delivery estimates, and costs for Cap Squeeze orders.">', f'<meta name="description" content="{desc}">')
...
print("Auth pages successfully fixed and regenerated!")
