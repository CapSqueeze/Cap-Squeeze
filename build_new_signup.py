...
# We want to replace everything between <!-- PAGE HERO --> and <!-- ========== FOOTER ========== -->
...
new_auth_content = '''
  <section style="min-height: 80vh; display: flex; align-items: center; justify-content: center; background: #fafafa;">
...
# Replace content
content = content[:start_idx] + new_auth_css + new_auth_content + '\n' + content[end_idx:]

# Also fix title
content = content.replace('<title>Contact Us — Cap Squeeze</title>', '<title>Sign Up — Cap Squeeze</title>')
content = content.replace('<title>Sign Up — Cap Squeeze</title>', '<title>Sign Up — Cap Squeeze</title>') # just in case

with open('signup.html', 'w', encoding='utf-8') as f:
    f.write(content)

print("signup.html properly rewritten with new design!")
